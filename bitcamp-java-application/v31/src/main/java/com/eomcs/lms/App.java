...
public class App {
  ...
  private void service() {
    // 애플리케이션의 서비스를 시작할때 등록된 옵저버에게 알린다 .
    for (ApplicationContextListener listener : appCtxListeners) {
      listener.contextInitalized(beanContainer);
    }
// 옵저버에게 보고한후 옵저버가 준비한 객체를 꺼낸다 
    ...
    // 애플리케이션의 서비스를 종료할때 등록된 옵저버에게 알린다 .
    for (ApplicationContextListener listener : appCtxListeners) {
  ...
  public static void main(String[] args) {
    App app = new App();
    // 애플리케이션을 시작하거나 종료할때 작업을 보고받고자 하는 객체를 종료한다 .
