...
// 별도의 실행 흐름을 만들기위해서는 Thread 클래스를 상속받아야한다 .
// 또는 Runnable 자격을 갖춘후 Thread로 실행해야한다
public class RequestHandler extends Thread {
  public static boolean isStopping = false;
  Socket socket;
 Map<String,Object> servletContext;
  // 클라이언트가 접속을하면 그 소켓 정보를 가지고 이 클래스의 인스턴스를 생성한다 . 
  // => 그런후 쓰레드를 분리하여 시작시키면, run() 이 실행된다 .
  ...
  // 별도의 실행흐름 (실:Thread)은 run() 메서드에 작성한다
  @Override
  public void run(){
    // 여기에서 클라이언트의 요청을 처리한다 .
    // main Thread와는 분리된 실행 흐름 (Thread)이기때문에
