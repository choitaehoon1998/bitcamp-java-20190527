...
// 
//
@HandlesTypes(MyWebInitalizer.class)
...
implements ServletContainerInitializer{
  ...
  public void onStartup(Set<Class<?>> types, ServletContext ctx) throws ServletException {
    ...
    if(types ==null)
      return;
    //이 라이브러리 외부에 구현된 MyWebInitalizer 구현체를 찾아 객체를 생성한다 . 
    for(Class<?> clazz : types) {
      try {
        MyWebInitalizer obj = (MyWebInitalizer) clazz.getConstructor().newInstance();
