...
public class Servlet01 extends HttpServlet {
  ...
  protected void service(
    ...
    // 테스트 방법:
    // => http://localhost:8888/bitcamp-java-web/ex09/s1  
    //
    
    // 1) ServletContext 보관소에 값 넣기
    //ServletContext 객체는 웹 어플리케이션이 시작될때 생성된다. 
    ServletContext sc = this.getServletContext();
    sc.setAttribute("v1", "aaa");
    
    // 2) HttpSession 보관소에 값 넣기
    // => HttpSession 객체는 웹 브라우저에서 '세션 아이디'를 제공하지않으면 
    //    getSession() 을 호출할때 생성된다. 
    //    이 요청을 한 클라이언트의 HttpSession 객체가 없다면 만들어준다.
    // => 웹브라우저에서 세션아이디를 제공하면 
    //    getSession()을 호출할때 기존에 생성했던 세션 객체를 리턴한다
    //    이미 이 클라이언트를 위해 만든 객체가 있다면 그 객체를 리턴한다.
    HttpSession session = request.getSession();
    session.setAttribute("v2", "bbb");
    
    // 3) ServletRequest 보관소에 값 넣기
    // => ServletRequest 객체는 클라이언트가 요청할때마다 생성된다 .
