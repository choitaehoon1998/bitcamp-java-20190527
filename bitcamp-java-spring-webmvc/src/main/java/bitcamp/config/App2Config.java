...
// => 지정된 패키지를 뒤져서 @Component , @Controller 등 붙은 클래스에대해 인스턴스를 생성한다 . 
@EnableWebMvc 
...
public class App2Config implements WebMvcConfigurer{
  ...
  //MatrixVariable 애노테이션의 처리를 활성화 시킨다. 
  ...
  // => 활성화 시켰으면, Ioc 컨테이너가 관리하는 객체중에서 
  //    WebMvcConfigurer 구현체를 찾는다 . 
  //    관리하는 객체?
  //    -Ioc 컨테이너에 들어 있는 객체 
  //    -@Component, @Service, @Controller, @RestController, @Repository
  //     애노테이션이 붙은 클래스들은 IocContainer 가 자동으로 객체를 생성하여 보관한다 
  //    -하지만 WebMvcConfigurer 구현체는 java Config 클래스이기때문에
  //     일반 객체로 표시하지말고, @Configuration 을 붙여
  //     java config 클래스임을 명확히 하는것이 유지보수에 좋다.
  //     물론, @Configuration 가 붙은 클래스도 객체가 생성되어 Ioc 컨테이너에 보관되는것은 같다 
  
  //    
  // => 객체를 찾았으면 ,WebMvcConfigurer 인터페이스 규칙에 따라 메서드를 호출하여 
  //    기존설정을 추가하거나 기존설정을 변경한다 . 
  // => WebMvc 설정을 활성화 시키지않으면 
  //    WebMvcConfigurer 구현체가있더라도 무시한다    
  // => WebMvc 설정을 활성화 시키는 방법은 
  //    1) xml 설정
  ...
  public void addInterceptors(InterceptorRegistry registry) {
    ...
    // => 인터셉터를 적용할 URL을 지정하지않으면 현재 프론트 컨트롤러의 모든 요청에 대해 적용된다 . 
    ...
    //    /c03_1/x
    //    /c04_1/a/x
    // 즉, /c04_1/ 바로밑에 있는 자원에 대해서만 인터셉터를 적용한다 . 
    ...
    //    적용 불가 : 
    //    /c03_1/x
    // 즉, /c04_1/ 모든 하위에 있는 자원에 대해서 인터셉터를 적용한다 . 
 ...
 // 4) 특정 요청에 대해 인터셉터 적용을 제외하기 .
    ...
    //    /c04_1/a/b/x
    // 즉, /c04_1/ 모든 하위에 있는 자원에 대해서 인터셉터를 적용한다 . 
    // 단 /c04_1/a/ 의 모든 하위 경로에있는 자원은 제외한다 . 
