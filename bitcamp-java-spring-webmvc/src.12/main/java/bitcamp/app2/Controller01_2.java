...
public class Controller01_2 {
  ...
  public void handler2(Model model) {
    ...
    // request handler의 URL을 뷰이름으로 사용한다 
    // 즉 이 핸들러의 URL은 "/c01_1/h2" 이기때문에 뷰이름도 /c01_2/h2가된다 .
    // InternalResourceViewResolver 는 바로 이 URL 을 사용하여 다음과같이 최종 url 을 만든다 . 
 ...
 public ModelAndView handler4() {
   ...
   //modleandview객체에 값과 뷰이름을 담아 리턴하면 
   //프론트컨트롤러는 modelandview 객체에 보관되어있는 값들을
   //ServletRequest 보관소로 옮기고
   // 설정된 뷰이름을 ViewResolver 에게 넘긴다 
   // 
 ...
 public ModelAndView handler5() {
   ...
   //프론트컨트롤러는 modelandview 객체에 보관되어있는 값들을
   //ServletRequest 보관소로 옮기고
   //뷰이름을 지정하지않으면 request handler의 path를 viewResolver 에게 넘긴다 . 
