...
// WebApplicationInitializer 구현체를 통해 DispactherServlet 을 등록하는  두 번째 방법
// => 인터페이스를 직접 구현하는 대신에 그 인터페이스를 구현한 
//    AbstractAnnotationConfigDispatcherServletInitializer 클래스를 상속 받기
//    -이 클래스는 미리 AnnotationConfigWebAppliciationContext Ioc 컨테이너를 준비했따
//    -따라서 Ioc 컨테이너를 따로 설정할 필요가없다 . 
//    -또한 DispactherServlet 을 등록하는 코드가 이미 작성되어있기때문에
//    -따로 등록할 필요가없다 . 
//    -즉 인터페이스를 직접 구현하는것 보다 편하다. 
