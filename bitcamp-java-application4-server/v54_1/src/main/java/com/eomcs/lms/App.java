// v54_1 : Apache의 HttpClient 를 이용하여  Http 서버 만들기 
...
public class App implements HttpRequestHandler{
  ...
  public void handle(HttpRequest request, HttpResponse response, HttpContext context)
    ...
    //커맨드 객체 애ㅔ잇는 requestHandler 를 호출할때 넘겨줄 파라미터 객체 준비
    ServletRequest servletRequest =new ServletRequest();
    ServletResponse servletResponse = new ServletResponse();
    //클라이언트가 요청한 명령알아내기 
    //[request line]
    // =>GET/member/add?name=aaa&email=aaa@test.com&password=1111&tel=1111-1111&age=20
    //[uri]
    // =>/member/add?name=aaa&email=aaa@test.com&password=1111&tel=1111-1111&age=20
    ...
    logger.info(command);
    if(values.length>1) {
    // => name=aaa&email=aaa@test.com&tel=1111-1111&age=20
    ...
    if (requestHandler != null) {
      ...
      servletRequest.setUri(uriStr); // URL 에 포함된 파라미타값을 추출하여 보관한다 . 
