...
//    - 프록시 패턴을 사용하면 이런 상황을 처리할수있다. 
// 프록시패턴
// => 다른객체에게는 원래 객체인 것처럼 보여야 하기 때문에
//    원래우의 객체와 같은 인터페이스를 구현한다
// => 그러나 실제일 을 할때는 자신이 하는것이아니라
//    원래 객체에게 떠님긴다 (위임)
// => 이때 위임하기전에 추가하거나 변경하고싶은 작업이있다면
//    처리하면 되는 것이다 
...
public class TxConnection implements Connection{
  ...
  //기존 메서드 중 close()대해서는 변경작업을 수행한다
  
  public void close() throws SQLException {
    //origin.close();
    // close()를 호출하더라도 닫지않는다.
    // realclose() 를 호출할때에 진짜 닫는다 . 
  }
  // TxConnection 에만있ㅇ는 메서드
