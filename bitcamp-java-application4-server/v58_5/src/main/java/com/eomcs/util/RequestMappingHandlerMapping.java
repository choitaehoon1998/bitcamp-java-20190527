...
public class RequestMappingHandlerMapping {
  ...
  private static final Logger log = LogManager.getLogger(RequestMappingHandlerMapping.class);
  ...
  public RequestMappingHandlerMapping(ApplicationContext iocContainer) {
    ...
    //각객체에대해 Request Handler 를 꺼낸다 . 즉 @RequestMapping 이붙은 객체를 꺼낸다 .
    for(Object obj : objects) {
      Set<Method> methods= getMethods(obj.getClass(),
  ...
  public static class RequestHandler {
    ...
    public Object invoke(HttpServletRequest request, HttpServletResponse response)throws Exception {
      ...
      // request Handler 를 호출하면 ,
      // model 객체에는 Request handler 가 담은값이 보관되어있다 
      // 여기에  Request Handler 의 리턴값 (Jsp Url) 도 함꼐 보관한다 . 
      model.put("viewUrl", viewUrl);

      //request handler 의 작업 결과물과 JSP URL 를 담은 맵객체를 프론트 컨트롤러에게 리턴한다 
      return model;
    }
    private List<String> getParameterNames (Class<?> type, Method method) throws Exception{
      // =>컴파일 할때 옵션으로 파라미터 이름을 포함하지 않는 한에는 
      //   자바  리플렉션 api 로는 알아낼수없다 
      // =>그런 .class 파일에는 분명히 파라미터 이름이 들어있다
      // => 그것을 꺼내려면 외부 라이브러리를 사용해야한다 .
    ...
    private Object[] getArguments (Parameter[]params,HttpServletRequest request, HttpServletResponse response, Map<String, Object> model)throws Exception {
      //파라미터 이름 알아내기 
    ...
    private Object getArgument(String paramName,Parameter param,HttpServletRequest request, HttpServletResponse response, Map<String, Object> model)throws Exception {
    ...
    @SuppressWarnings("unchecked")
    private Object getPojoValue(String paramName, Class<?> paramType, HttpServletRequest request)throws Exception {
      // POJO 클랫의 메서드를 추출한다 
      ...
      // =>클라이언트가 보낸 데이터중에서 세터 메서드의 넘겨줄 요청 데이터가 있다면 호출한다 . 
      for(Method m : methods) {
        // 메서드 이름에서 프로퍼티 명을 추출한다 .
        String propName = m.getName().substring(3,4).toLowerCase()+
            m.getName().substring(4); //SetCreatedDate => CreatedDate
         log.debug("             " + propName);
        // 클라이언트가 보낸 값 중에서 파라미터 이름과 일치하는 
       
       //클라이언트가 보낸 데이터중에서 프로퍼티 이름과 일치하는 데이터가있다면 꺼낸다 . 
        
        // 세터를 호출할때 넘겨줄 값을 담을 변수를 준비 
       Object value =null;
      ...
      value  = getPrimitiveValue(propName, m.getParameters()[0].getType(), request);
        ...
        // 세터에 넘겨줄 값을 준비하지 못했으면 다음메서드로 넘어간다 . 
       if(value ==null) 
         continue;
