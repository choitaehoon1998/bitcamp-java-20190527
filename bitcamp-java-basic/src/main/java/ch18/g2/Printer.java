...
public interface Printer {
  // 일반적으로 선언하는 인터페이스의 메서드는 public abstract 메서드이다.
  // 따라서 이 인터페이스를 구현하는 모든 클래스가 이메서드를 정의해야 한다.

  void print(String text);


  // 다음과같이 새기능을 무심코 추가한다면
  // 기존에 이인터페이스에 규칙에 따라 작성한 모든 클래스들에
  // 컴파일 오류가 발생할것이다
  // 왜? 새 규칙이 추가되었기 때문이다
  // 새규칙이 추가되었으면 기존 클래스돌도 새 규칙을 구현해야한다 .
  //
  ...
  // 디폴트 메서드를 사용하면 기존 구현체(인터페이스를 구현한 클래스) 에 영향을 끼치지않으면서
  // 새 규칙을 추가할수있따
