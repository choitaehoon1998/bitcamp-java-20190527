...
public class MyAutowiredAnnotationBeanPostProcessor implements BeanPostProcessor {
  ...
  private void addBean(Class<?> type, Object bean) {
    ...
    if(objList ==null) {    // 해당 타입의 객체 목록이없다면 
      objList =new ArrayList<>(); // 새로객체를 저장할 목록을 준비한다 . 
      beans.put(type,objList); // 해당 타입의 객체를 저장할 목록을 맵에 등록, 추가한다 
    }
    objList.add(bean);//해당타입의 객체 목록에 새객체를 추가한다 .
  }
  private Object getBean(Class<?>type ) {
    List<Object> objList =beans.get(type);
    if(objList ==null) {    // 해당 타입의 객체 목록이없다면 
      return null;          //null 를리턴
    }
    return objList.get(0);//객체목록에서 제일 첫번째 객체를 꺼낸다.
  }
  ...
  public Object postProcessAfterInitialization(Object bean, String beanName) throws BeansException {
    ...
    //해당빈에서 @Autoweird가 붙은 메서드를 찾아 호출하거나 호출을 연기시킨다 . 
 ...
 // 이 객체의 모든 public 메서드를 꺼낸다.
  ...
  private void injectDependency(Object dependency) {
    //파라미터로 받은 객체를 원하는 세터가 있는지 확인한다 . 
    //즉  이타입의 객체를 받기위해 호출이 연기된 메서드가 있는지 확인한다 . 
    List<AutowiredMethod> setters = autowiredMethods.get(dependency.getClass());

    if (setters == null) // 없다면 호출하지 않는다.
      return;
    // 이객체를 간절히 원하는 세터 메스드가 있다면 호출하여 주입한다 
