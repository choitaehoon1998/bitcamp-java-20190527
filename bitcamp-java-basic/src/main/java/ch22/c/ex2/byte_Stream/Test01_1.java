...
public class Test01_1 {
  public static void main(String[] args) throws Exception {
    ...
    // => 상위 바이트를 출력하고싶으면 맨 뒤로 이동시켜야 한다 .
    // => >> 연산자를 사용하여 비트를 이동시킨다. 
    // => 빈자리는 부호 비트로 채워진다 . 양수이면 0 음수이면 1로 채운다 . 
    // => 맨 끝에서 넘어간 바이트는 버려진다 . 
    out.write(value >> 24); // 00 00 00 22 !    | 33 44 55 버려짐
    out.write(value >> 16); // 00 00 22 33 !    | 44 55 버려짐 
    out.write(value >> 8);  // 00 22 33 44 !    | 55 버려짐
    out.write(value);       // 22 33 44 55 !    | 
    
    // Primitive 타입의 값을 출력하기위해
    // 매번 위코드 처럼 작성하는것은 번거로운일이다 . 
    // 이코드를 캡슐화하여 별도의 클래스로 정의해보자 !
